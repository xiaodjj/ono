...
import moe.ono.ui.ResUtils;
import moe.ono.util.HostInfo;
import moe.ono.util.Initiator;
import moe.ono.util.Logger;
import moe.ono.util.consis.StartupDirectorBridge;
...
public abstract class BaseActivity extends AppCompatTransferActivity {
            ...
            try {
                if (mLifecycleCallbacks != null && !mLifecycleCallbacks.isEmpty()) {
                    for (Object cb : mLifecycleCallbacks) {
                        // Logger..d("invoke instance callback " + Reflex.getShortClassName(cb) + "." + callback.getName());
                ...
                if (globalCallbacks != null && !globalCallbacks.isEmpty()) {
                    for (Object cb : globalCallbacks) {
                        // Logger..d("invoke global callback " + Reflex.getShortClassName(cb) + "." + callback.getName());
                        callback.invoke(cb, args);
                    }
                }
                ...
                Logger.e("invokeCallback: " + callback, cause);
            }
        }
    }
    ...
    private static void initLifecycleHooks() {
        ...
        try {
            ...
            Logger.e("LifecycleHook: missing methods or fields", e);
        }
    }
    ...
    private ArrayList<?> createInstanceLifecycleCallbacks() {
        ...
        for (Class<?> instanceLifecycleCallbacksClass : instanceLifecycleCallbacksClasses) {
            try {
                ...
                Logger.e("LifecycleHook: failed to create instance of " + instanceLifecycleCallbacksClass, e);
            }
        }
        return callbacks;
    }
